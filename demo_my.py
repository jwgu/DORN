...
#parser.add_argument('--filename', type=str, default='./data/kihwan_mixed/demo_01.png', help='path to an image')
parser.add_argument('--outputroot', type=str, default='./result/kihwan_mixed', help='output path')

test_imgfiles = os.listdir('./data/kihwan_mixed')
test_imgfiles = [ './data/kihwan_mixed/'+f for f in test_imgfiles]
...
def depth_prediction(filename):
    ...
    img = cv2.resize(img, (int(0.1*W), int(0.1*H))) # resize to 1/10x size
